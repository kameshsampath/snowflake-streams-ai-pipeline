...
## Setup Service User for Openflow
snow sql --stdin <<EOF
    use role accountadmin;
    CREATE USER IF NOT EXISTS $SA_USER
        TYPE = SERVICE
        COMMENT = 'Service User For Openflow';
    GRANT ROLE $SNOWFLAKE_OPENFLOW_DEMO_ROLE TO USER $SA_USER;
    GRANT APPLY AUTHENTICATION POLICY ON USER $SA_USER TO ROLE $SNOWFLAKE_OPENFLOW_DEMO_ROLE;
EOF
...
snow sql --stdin <<EOF
use role $SNOWFLAKE_OPENFLOW_DEMO_ROLE;
use database $SNOWFLAKE_OPENFLOW_DEMO_DATABASE;
...
drop network policy if exists "${SA_USER}_LOCAL_PAT_NETWORK_POLICY";
use role $SNOWFLAKE_OPENFLOW_DEMO_ROLE;
-- create or replace network rule to allow local machine IPv4 addresses
create or replace network rule $SNOWFLAKE_OPENFLOW_DEMO_DATABASE.networks.pat_local_access_rule
  mode = ingress
  type = ipv4
...
use role accountadmin;
create network policy "${SA_USER}_LOCAL_PAT_NETWORK_POLICY"
allowed_network_rule_list = ('"$SNOWFLAKE_OPENFLOW_DEMO_DATABASE".networks.pat_local_access_rule')
comment = 'Network policy to allow all IPv4 addresses.';
alter user "${SA_USER}" set network_policy="${SA_USER}_LOCAL_PAT_NETWORK_POLICY";
EOF
...
snow sql --stdin <<EOF
use role $SNOWFLAKE_OPENFLOW_DEMO_ROLE;
GRANT USAGE ON DATABASE $SNOWFLAKE_OPENFLOW_DEMO_DATABASE TO ROLE ACCOUNTADMIN;
GRANT USAGE ON SCHEMA $SNOWFLAKE_OPENFLOW_DEMO_DATABASE.policies TO ROLE ACCOUNTADMIN;
GRANT USAGE ON SCHEMA $SNOWFLAKE_OPENFLOW_DEMO_DATABASE.networks TO ROLE ACCOUNTADMIN;
create or replace authentication policy $SNOWFLAKE_OPENFLOW_DEMO_DATABASE.policies.${USER}_openflow_auth_policy
  ...
  alter user $SA_USER set AUTHENTICATION POLICY $SNOWFLAKE_OPENFLOW_DEMO_DATABASE.policies.${USER}_openflow_auth_policy;
EOF
...
EXISTING_PAT=$( snow sql -q "show user pats for user $SA_USER" \
  --format=json \
  | jq -r '.[] | select(.name|ascii_downcase == "openflow_demo") | .name')
...
if [ -z "$EXISTING_PAT" ]; then
  ...
  SNOWFLAKE_SA_PASSWORD=$(snow sql --role="accountadmin" \
    --query "ALTER USER IF EXISTS $SA_USER ADD PAT openflow_demo ROLE_RESTRICTION = $SNOWFLAKE_OPENFLOW_DEMO_ROLE" \
  ...
  SNOWFLAKE_SA_PASSWORD=$(snow sql \
    --query "ALTER USER IF EXISTS $SA_USER ROTATE PAT openflow_demo" \
    --format=json | jq -r '.[] | .token_secret')
fi;
